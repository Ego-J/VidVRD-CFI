...
from utils.utils import get_feat_types, vru19_ext_loc_feat, aaai18_ext_mask_feat, gen_union_bbox
...
class FeatExtractor:
    ...
    #     for i in range(30):
    #         if i < clip_len:
    #             rel_feats.append(vru19_ext_loc_feat(data['sbj_traj'][i], data['obj_traj'][i], self.video_height, self.video_width))
    #         else:
    #             rel_feats.append(np.zeros(14))
    #     feat = np.concatenate((rel_feats),-1)
    #     return feat
    # def _extract_mot_feat(self):
    #     data = self.data
    #     assert data != None
    ...
    #         else:
    #             rel_feats.append(np.zeros(14))
    #     feats = np.asarray(rel_feats)
    #     feats_3frames = (feats[range(2, 30, 3)] - feats[range(0, 30, 3)]).reshape(1,-1)
    #     feats_5frames = (feats[range(2, 30, 5)] - feats[range(0, 30, 5)]).reshape(1,-1)
    #     feats_10frames = (feats[range(2, 30, 10)] - feats[range(0, 30, 10)]).reshape(1,-1)
    #     feats_30frames = (feats[-1] - feats[0]).reshape(1,-1)
    #     feat = np.concatenate((feats_3frames,feats_5frames,feats_10frames,feats_30frames),-1).reshape(-1)
    #     return feat
    ...
    #     data = self.data
    #     assert data != None
    #     feat = np.asarray([0.0]*4*CLIP_LEN).reshape(-1,4)
    #     for fno in range(0, len(data['sbj_traj'])):
    #         feat[fno] = vtranse_ext_loc_feat(data['sbj_traj'][fno], data['obj_traj'][fno])
    #     feat = feat.reshape(-1)
    #     return feat
    # def _extract_mot_feat(self):
    #     feat = self._extract_rel_feat()
    #     feat = feat[4:] - feat[:-4]
    #     return feat
...
def gen_label(clip, gt_relations):
    ...
    for relation in gt_relations:
        if (clip["sbj_tid"] == relation["subject_tid"]) and (clip["obj_tid"] == relation["object_tid"]):
            #1 clipment has the relation label if having intersection
...
def gen_feats(split, traj_source):
    ...
    feat_path = split + '_' + traj_source + '_' + str(CLIP_LEN) + '_debug'
...
if __name__ == '__main__':
    ...
    # gen_feats("test", traj_source="meta")
    # gen_feats("test", traj_source="meta_d5")

    gen_feats("train", traj_source="gt")
    # gen_feats("val", traj_source="meta")
